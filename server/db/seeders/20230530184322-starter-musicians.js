...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    for (let bandIdx = 0; bandIdx < bandMusicians.length; bandIdx++) {
      ...
      for (let musicianIdx = 0; musicianIdx < musicians.length; musicianIdx++) {
        ...
        await Musician.create({ firstName: musician.firstName, lastName: musician.lastName, bandId: band.id });
      }
    }
  },
  ...
  down: async (queryInterface, Sequelize) => {
    for (let bandIdx = 0; bandIdx < bandMusicians.length; bandIdx++) {
      ...
      for (let musicianIdx = 0; musicianIdx < musicians.length; musicianIdx++) {
        const musician = musicians[musicianIdx];
        await Musician.destroy({ where: { firstName: musician.firstName, lastName: musician.lastName, bandId: band.id } });
    }
  }
}
};
